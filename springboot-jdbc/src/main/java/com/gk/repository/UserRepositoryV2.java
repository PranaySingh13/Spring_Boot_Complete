...
public class UserRepositoryV2 {
	...
	public void addUser(User user) {
		try {
			 ...
			 * .addValue("uid",user.getId()) .addValue("uname", user.getName())
			 * .addValue("uage",user.getAge()) .addValue("ucity", user.getCity());
			 * 
	...
	public List<User> getAllUsers() {
		...
		try {
			...
			users = (List<User>) responseMap.get(users);
	...
	public String getUserName(int userId) {
		...
		try {
			simpleJdbcCall = new SimpleJdbcCall(jdbcTemplate).withSchemaName("springbootjdbcdb")
					.withFunctionName("get_user_name");
			SqlParameterSource params = new MapSqlParameterSource()
					.addValue("uid",userId);
			name = simpleJdbcCall.executeFunction(String.class,params);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return name;
